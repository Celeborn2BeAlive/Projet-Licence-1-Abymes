...
/*FONCTION CHARGEANT L'INTERFACE PUREMENT CONSOLE*/
/*Utilisée uniquement pour les premiers test, ne comporte pas toutes les fonctionnalités, telle que la sauvegarde. En eviter l'utilisation.*/
...
/*FONCTION AFFICHANT LA CARTE EN CONSOLE*/
...
void affichageConsole(int** carte, mobile joueur, int nbre_lignes, int nbre_colonnes)
{
    ...
    for(i = 0; i < nbre_lignes; i ++) /*on parcours toutes les lignes*/
    {
        for(j = 0; j < nbre_colonnes; j ++) /*on parcours toutes les colonnes*/
        {
            if(i == joueur.coords.y && j == joueur.coords.x) /*si on arrive a la position du joueur*/
                printf("%c", joueur.name[0]); /*on affiche la premiere lettre de son nom*/
            else /*sinon*/
                printf("%c", carte[i][j]); /*on affiche la carte*/
        }
        printf("\n"); /*saut de ligne*/
    }
}
