...
point* initPerm(int* permutation, point* s)
{
	...
	for(i = 0; i < 32; i++) /*pour chacun des points de la scene*/
		...
		/*on place son point permuté à sa position dans le tableau*/
...
{
	/*Nous avons utilisé une base differente de celle de l'ecran pour faciliter les calculs, cette fonction effectue le changement de base de notre*/
	/*repere vers l'ecran afin de pouvoir afficher la scene.*/

	point Oprim = initPoint(237.5, 350 - LARGEUR/3.33); /*Origine du repere utilisé lors des calculs, correspond au point s[0]*/
...
/*se referer au shema fourni avec le projet pour une vue d'ensemble de la scene et ainsi comprendre les calculs effectuées*/
